...
getMyEl("getResult").addEventListener("click", function () {
  ...
  if (result) {
    ...
    if (sum % 1 === 0) {
      getMyEl("resultEx2").innerHTML = "Tổng của " + number1 + " và " + number2 + " có dạng x^1 + x^2 + ... + x^n: " + sum;
    } else {

      getMyEl("resultEx2").innerHTML = "Tổng của " + number1 + " và " + number2 + " có dạng x^1 + x^2 + ... + x^n: " + sum.toFixed(2);
    }
  } else {
